runCellChat<-function(loaded.dataSO.combined,labelsC,cellIDs){
  ...
  for(labelIndex in LablesUniq){
    expr <- FetchData(loaded.dataSO.combined, vars = labelsC)
    loaded.dataSO.combinedSub <- loaded.dataSO.combined[, which(expr == labelIndex)]

    data.input<-GetAssayData(object = loaded.dataSO.combinedSub, slot = "data",assay = "RNA")#Does this need to be RNA check

    cell.use = names(loaded.dataSO.combinedSub$orig.ident)
    ...
    #meta = meta[cell.use, ]
    #or this
    loaded.dataSO.combinedSub$celltype2<-loaded.dataSO.combinedSub[[cellIDs]][,1]

    meta = data.frame(labels = loaded.dataSO.combinedSub$celltype2[cell.use], row.names = colnames(data.input)) # manually create a dataframe consisting of the cell labels
